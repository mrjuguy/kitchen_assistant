...
const mapOFFCategory = (offCategories: string[] = []): string => {
  if (
    offCategories.includes("en:fruits") ||
    offCategories.includes("en:vegetables")
  )
    return "Produce";
  if (offCategories.includes("en:dairies")) return "Dairy";
  if (
    offCategories.includes("en:meats") ||
    offCategories.includes("en:proteins")
  )
    return "Protein";
  ...
  if (
    offCategories.includes("en:pastries") ||
    offCategories.includes("en:bakery-products")
  )
    return "Bakery";
  return "Pantry";
};
      ...
      return {
        ...
        quantity: (() => {
          ...
          if (product.product_quantity)
            return parseFloat(product.product_quantity);
...
export const searchProducts = async (query: string): Promise<ProductData[]> => {
  ...
  try {
    const response = await fetch(
      `https://us.openfoodfacts.org/cgi/search.pl?search_terms=${encodeURIComponent(query)}&search_simple=1&action=process&json=1&page_size=10`,
    );
    const data = await response.json();
