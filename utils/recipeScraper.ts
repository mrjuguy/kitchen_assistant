 ...
 */
export async function extractRecipeFromUrl(url: string): Promise<ScrapedRecipe | null> {
    try {
...
function mapJsonLdToRecipe(data: any, sourceUrl: string): ScrapedRecipe {
    ...
    if (data.recipeInstructions) {
        if (Array.isArray(data.recipeInstructions)) {
            instructions = data.recipeInstructions.map((i: any) => {
    ...
    return {
        ...
        tags: data.keywords ? (typeof data.keywords === 'string' ? data.keywords.split(',') : data.keywords) : [],
