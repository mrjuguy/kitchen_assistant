...
export function convertUnit(value: number, fromUnit: UnitKey, toUnit: UnitKey): number {
    ...
    if (fromDef.category !== toDef.category) {
        console.warn(`Cannot convert between categories: ${fromDef.category} -> ${toDef.category}`);
        return value;
    }
 ...
 */
export function normalizeToUS(value: number, unit: UnitKey): { value: number; unit: UnitKey } {
    ...
    for (const target of targets) {
        ...
        if (Math.abs(converted - Math.round(converted)) < 0.1 && Math.round(converted) > 0) {
...
export function getBaseUSUnit(category: UnitCategory): UnitKey {
    switch (category) {
        case 'volume': return 'fl_oz';
        case 'weight': return 'oz';
        default: return 'unit';
    }
}
