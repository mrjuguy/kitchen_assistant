import { extractRecipeFromUrl } from './recipeScraper';
...
describe('extractRecipeFromUrl', () => {
  ...
  it('extracts a simple recipe correctly', async () => {
    ...
    expect(recipe?.ingredients[0]).toEqual({ name: 'eggs', quantity: 2, unit: 'large' }); // "2 large eggs" -> qty:2, unit:large, name:eggs ? actually my parser is primitive
    // Wait, "2 large eggs" -> 
    // Regex: (2) (large) (eggs)
...
import { parseIngredient, parseIsoDuration } from './recipeScraper';

describe('parseIngredient', () => {
  it('parses simple quantity unit name', () => {
    expect(parseIngredient('1 cup flour')).toEqual({ quantity: 1, unit: 'cup', name: 'flour' });
  });

  it('parses fractions', () => {
    expect(parseIngredient('1/2 tsp salt')).toEqual({ quantity: 0.5, unit: 'tsp', name: 'salt' });
  });

  it('parses decimals', () => {
    expect(parseIngredient('1.5 kg chicken')).toEqual({ quantity: 1.5, unit: 'kg', name: 'chicken' });
  });

  it('normalizes units', () => {
    expect(parseIngredient('2 Tablespoons sugar')).toEqual({ quantity: 2, unit: 'tbsp', name: 'sugar' });
  });

  it('handles no quantity (defaults to 1)', () => {
    expect(parseIngredient('salt')).toEqual({ quantity: 1, unit: 'unit', name: 'salt' });
  });
});
