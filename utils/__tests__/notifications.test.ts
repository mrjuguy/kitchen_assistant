...
describe("scheduleExpiryNotification", () => {
  ...
  it("should schedule both warning and critical alerts for items expiring in 10 days", async () => {
    ...
    ).mock.calls.find((call) => call[0].content.title === "Freshness Warning");
    expect(warningCall).toBeTruthy();
    expect(warningCall[0].trigger.date.toDateString()).toBe(
      expectedWarningDate.toDateString(),
    );
    ...
    const criticalCall = (
      ...
      (call) => call[0].content.title === "Critically Low Freshness",
    );
    expect(criticalCall).toBeTruthy();
    expect(criticalCall[0].trigger.date.toDateString()).toBe(
      expectedCriticalDate.toDateString(),
    );
  });
