...
describe('recipeFilters', () => {
    describe('filterRecipes', () => {
        ...
        it('filters by status: ready', () => {
            const result = filterRecipes(mockRecipes, { statusFilter: 'ready', analysisMap: mockAnalysis });
            expect(result).toHaveLength(1);
            expect(result[0].id).toBe('1');
        });

        it('filters by status: missing', () => {
            const result = filterRecipes(mockRecipes, { statusFilter: 'missing', analysisMap: mockAnalysis });
        ...
        it('filters by multiple tags (OR logic)', () => {
            const result = filterRecipes(mockRecipes, { tags: ['Italian', 'Healthy'] });
            expect(result).toHaveLength(2);
    ...
    describe('getAvailableTags', () => {
        it('extracts unique tags correctly', () => {
            ...
            expect(tags).toEqual(['Breakfast', 'Dinner', 'Healthy', 'Italian', 'Keto']);
        });
    });
});
