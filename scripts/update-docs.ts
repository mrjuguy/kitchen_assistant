...
// Configuration
const DOCS_TO_UPDATE = ["README.md", "specs/tech-stack.md"];

async function getGitContext() {
  try {
    // Get the last commit message and the stat (files changed)
    const commitInfo = execSync("git log -1 --stat", {
      encoding: "utf-8",
    ...
    const diff = execSync('git show HEAD --pretty=""', {
      encoding: "utf-8",
      maxBuffer: 1024 * 1024 * 10,
    }).slice(0, 4000);
...
async function updateDocument(filePath: string, gitContext: string) {
  ...
  try {
    const message = await anthropic.messages.create({
      ...
      max_tokens: 4096,
      system:
        "You are 'The Scribe', a technical documentation bot. Your goal is to keep documentation 100% in sync with code. You will be given a file and a set of code changes. You must update the file to reflect the changes. Return ONLY the updated file content. Do not include markdown code fences (```) or conversational text.",
      messages: [
        {
          role: "user",
          content: `
