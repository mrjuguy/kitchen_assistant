...
export default function CreateRecipeScreen() {
    ...
    const handleImport = async () => {
        ...
        if (!recipe) {
            Alert.alert("Import Failed", "Could not extract recipe details. Please try another URL or enter manually.");
            return;
        }
    ...
    return (
        <SafeAreaView style={{ flex: 1, backgroundColor: 'white' }}>
            ...
            <ScrollView contentContainerStyle={{ padding: 24 }}>
                {/* Import Section */}
                <View style={{ backgroundColor: '#f0fdf4', padding: 16, borderRadius: 12, marginBottom: 24, borderWidth: 1, borderColor: '#dcfce7' }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
                        <LinkIcon size={16} color="#15803d" />
                        <Text style={{ marginLeft: 8, color: '#15803d', fontWeight: 'bold' }}>Import from Web</Text>
                    </View>
                    <View style={{ flexDirection: 'row' }}>
                        ...
                        >
                            {isImporting ? <ActivityIndicator color="white" size="small" /> : <Download size={20} color="white" />}
                        </Pressable>
                    </View>
                </View>
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 24, marginBottom: 8 }}>
                    ...
                    <Pressable onPress={addInstructionStep} style={{ flexDirection: 'row', alignItems: 'center' }}>
                        <Plus size={16} color="#10b981" />
                        <Text style={{ color: '#10b981', fontWeight: 'bold', marginLeft: 4 }}>Add Step</Text>
                    </Pressable>
                </View>
                ...
                {instructions.map((item, index) => (
                    <View key={item.id} style={{ flexDirection: 'row', marginBottom: 12 }}>
                        <View style={{ width: 30, paddingTop: 12 }}>
                            <Text style={{ fontWeight: 'bold', color: '#6b7280' }}>{index + 1}.</Text>
                        </View>
                        <View style={{ flex: 1, marginRight: 8 }}>
                            <TextInput
                                ...
                                multiline
                                style={{ backgroundColor: '#f9fafb', padding: 12, borderRadius: 8, minHeight: 60, textAlignVertical: 'top' }}
                            />
                        </View>
                        <Pressable onPress={() => removeInstructionStep(item.id)} style={{ padding: 8, paddingTop: 12 }}>
                            <Trash2 size={20} color="#ef4444" />
                        </Pressable>
                    </View>
                ))}
