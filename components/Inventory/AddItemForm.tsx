...
export const AddItemForm: React.FC<AddItemFormProps> = ({
  ...
  >(undefined);
  const [ingredientsText, setIngredientsText] = useState<string | undefined>(
    undefined,
  );
  ...
  const handleBarcodeScanned = async ({ data }: { data: string }) => {
    ...
    if (product) {
      ...
      if (product.quantity && product.unit) {
        const { value: normQty, unit: normUnit } = normalizeToUS(
          product.quantity,
          product.unit as any,
        );
      ...
      Alert.alert(
        "Product Not Found",
        "Could not find product info for this barcode. You can still enter details manually.",
        [{ text: "OK" }],
      );
    }
  };
      ...
      >
        ...
        <View className="mb-8 flex-row bg-slate-100 dark:bg-slate-800 p-1 rounded-2xl">
          <Pressable
            ...
            className={`flex-1 flex-row items-center justify-center gap-2 py-3 rounded-xl ${
              target === "pantry" ? "bg-white dark:bg-slate-700" : ""
            }`}
          >
            <PackageOpen
              size={20}
              color={target === "pantry" ? "#0d7ff2" : "#94a3b8"}
            />
            <Text
              className={
                target === "pantry"
          ...
          <Pressable
            ...
            className={`flex-1 flex-row items-center justify-center gap-2 py-3 rounded-xl ${
              target === "shopping" ? "bg-white dark:bg-slate-700" : ""
            }`}
          >
            <ShoppingCart
              size={20}
              color={target === "shopping" ? "#0d7ff2" : "#94a3b8"}
            />
            <Text
              className={
                target === "shopping"
        ...
        <View className="mt-4 mb-8">
          <Pressable
            ...
            disabled={
              addPantryMutation.isPending ||
              addShoppingMutation.isPending ||
              !name.trim()
            }
            className={`w-full py-4 rounded-2xl flex-row items-center justify-center gap-3 ${
              addPantryMutation.isPending ||
              addShoppingMutation.isPending ||
              !name.trim()
          ...
          <Text className="text-center text-xs text-slate-400 mt-3 font-medium">
