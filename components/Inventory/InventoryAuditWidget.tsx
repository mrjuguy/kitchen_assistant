...
export const InventoryAuditWidget: React.FC = () => {
  ...
  const currentItem = staleItems.find(
  ...
  const confirmStock = (fraction: number) => {
    ...
    if (fraction === 0) {
      ...
      updateMutation.mutate({
        id: currentItem.id,
        updates: { quantity: 0, updated_at: new Date().toISOString() },
      });
    } else if (fraction === 1) {
  ...
  return (
    <View
      className="bg-blue-50 rounded-2xl p-4 mb-4 border border-blue-200 shadow-sm"
      style={{ shadowColor: "#3b82f6", elevation: 2 }}
    >
      ...
      <Text className="text-blue-900 mb-4 leading-5">
        You haven't updated{" "}
        <Text className="font-bold">{currentItem.name}</Text> in a while. Do you
        still have roughly {Math.round(currentItem.quantity * 10) / 10}{" "}
        {currentItem.unit}?
      </Text>
